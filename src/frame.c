...
LRESULT CALLBACK ApplicationOptionDlgProc(HWND hDlg, UINT message, WPARAM wParam,
                                          LPARAM lParam) {
    switch (message) {
        case WM_INITDIALOG: {
            BOOL fCheck;
...
#define OPTPS_NUM_PAGES 4
    LPTSTR lpTitle[OPTPS_NUM_PAGES] = {
            STRING_OPTIONDIALOG_EMULATOR, STRING_OPTIONDIALOG_EDITOR,
            STRING_OPTIONDIALOG_APPLICATION, STRING_OPTIONDIALOG_OBJECTVIEW};
    LPTSTR lpDlgResName[OPTPS_NUM_PAGES] = {
            __T("EMULATOROPTIONDLG"), __T("EDITOROPTIONDLG"),
            __T("APPLICATIONOPTIONDLG"), __T("OBJECTVIEWOPTIONDLG")};
    DLGPROC pfnDlgProc[OPTPS_NUM_PAGES] = {EmulatorOptionDlgProc, EditorOptionDlgProc,
                                           ApplicationOptionDlgProc,
                                           ObjectViewOptionDlgProc};
...
void RefreshWindowTitle(BOOL fNeedChanged) {
    ...
    if (fNeedChanged) lstrcat(cWndTitle, __T("*"));
...
BOOL fr_GetDataChanged() { return gblDataChanged; }
...
#define STBBS_NUM_BUTTONS 9
    ...
    if (gblIsROMLoaded) {
        ...
        for (i = 0; i < STBBS_NUM_BUTTONS; i++)
            SendMessage(hTBWnd, TB_SETSTATE, uMenuID[i],
...
static HWND CreateMainWindowToolBar(HWND hWnd) {
    ...
    int idCommand[TOOLBAR_BUTTONS] = {IDM_FILE_OPEN,
                                      IDM_FILE_SAVE,
                                      0,
                                      IDM_SETTING_AREA,
                                      0,
                                      ...
                                      IDM_EMULATOR_STOP,
                                      IDM_EMULATOR_TESTPLAYSETTING};
    BYTE fsState[TOOLBAR_BUTTONS] = {TBSTATE_ENABLED,
                                     TBSTATE_ENABLED,
                                     0,
                                     TBSTATE_ENABLED,
                                     0,
                                     ...
                                     TBSTATE_ENABLED,
                                     TBSTATE_ENABLED};
    BYTE fsStyle[TOOLBAR_BUTTONS] = {
            TBSTYLE_CHECK,  TBSTYLE_BUTTON,     TBSTYLE_SEP,        TBSTYLE_CHECK,
            TBSTYLE_SEP,    TBSTYLE_CHECKGROUP, TBSTYLE_CHECKGROUP, TBSTYLE_SEP,
    ...
    hWndTb =
            CreateToolbarEx(hWnd, WS_VISIBLE | WS_CHILD | TBSTYLE_TOOLTIPS, IDW_TOOLBAR,
                            TOOLBAR_BUTTONS, GetModuleHandle(NULL), TOOLBAR_IMG, tbBtn,
                            TOOLBAR_BUTTONS, 0, 0, 16, 16, sizeof(TBBUTTON));
...
void UpdateStatusBarRoomInfoText(HWND hSbWnd) {
    ...
    if (gblIsROMLoaded) {
        ...
        wsprintf(szText, __T(" %d-%d ( %.2XH [ $%.4X, $%.4X ] )"), g_iWorld + 1,
                 g_iArea + 1, GetRoomID(), wAddrBadGuys, wAddrMap);
    } else {
        szText[0] = 0;
    }
...
#define SB_EXTRASPACE 1
    ...
    SendMessage(hStatusBarWnd, SB_SETPARTS, sizeof(iSize) / sizeof(iSize[0]),
                (LPARAM)(LPINT)iSize);
}
...
static HWND CreateMainWindowStatusBar(HWND hWnd) {
    ...
    hWndSb = CreateStatusWindow(WS_CHILD | WS_VISIBLE, NULL, hWnd, IDW_STATUSBAR);
...
BOOL LoadCHRROMFromFile(LPTSTR pFileName, BOOL fLoadAll) {
    ...
    if (memcmp(CHRHead.cType, "NES\x1a", 4) ||
        (CHRHead.bNum_CHARs != SMB_NUM_CHARS || CHRHead.bNum_PRGs != SMB_NUM_PRGS)) {
    ...
    wCHRValidSize =
            (!fLoadAll) ? bPRGROM[0x870A] | ((WORD)bPRGROM[0x8705] << 8) : SMB_CHR_SIZE;

    if (wCHRValidSize > SMB_CHR_SIZE) wCHRValidSize = SMB_CHR_SIZE;
...
BOOL SetSaveFileName(HWND hWnd) {
    ...
    fname.Flags = OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER |
                  OFN_ENABLEHOOK | OFN_NOREADONLYRETURN | OFN_NOCHANGEDIR;
    fname.lpstrDefExt = __T("nes");
...
void DumpVersion() {
    ...
    wsprintf(szBuf, __T("----------------- %s Ver.%d.%.2d -----------------"),
             PROGRAMNAME, MAJORVERSION, MINORVERSION);
    lv_OutputString(szBuf, LOGVIEW_OUTPUTSTRING_CR);
...
LRESULT CALLBACK VersionDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        case WM_INITDIALOG: {
            ...
            wsprintf(szBuf, __T("%s Ver.%d.%.2d"), PROGRAMNAME, MAJORVERSION,
                     MINORVERSION);
...
static void ResizeMDIParentWindow(HWND hWnd) {
    ...
    if (lv_ShowLogView()) {
        MoveWindow(lv_GetBorderWnd(), 0,
                   wHeight - iStatusY - (iLogViewY + LOGVIEW_BORDERWIDTH), wWidth,
                   LOGVIEW_BORDERWIDTH, TRUE);
        MoveWindow(lv_GetLogViewWnd(), 0, wHeight - iStatusY - iLogViewY, wWidth,
                   iLogViewY, TRUE);
        iLogViewY += LOGVIEW_BORDERWIDTH;
    }
    MoveWindow(ghWndMDIClient, 0, iToolY, wWidth,
               wHeight - iToolY - iStatusY - iLogViewY, TRUE);
...
#define SMSNR_NUM_MENUS 24
    ...
    if (gblIsROMLoaded) {
        ...
        uEnable = (IsEmulatorRunning()) ? (MF_BYCOMMAND | MF_ENABLED)
                                        : (MF_BYCOMMAND | MF_GRAYED);

        EnableMenuItem(hMenu, IDM_EMULATOR_SAVE, uEnable);
        EnableMenuItem(hMenu, IDM_EMULATOR_STOP, uEnable);

        uEnable = (IsEmulatorSavePresent()) ? (MF_BYCOMMAND | MF_ENABLED)
                                            : (MF_BYCOMMAND | MF_GRAYED);
        EnableMenuItem(hMenu, IDM_EMULATOR_LOAD, uEnable);

        uEnable = (rm_IsSubRoom()) ? (MF_BYCOMMAND | MF_GRAYED)
...
LRESULT APIENTRY MDIFrameWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        ...
        case WM_NOTIFY: {
            ...
            if (g_hTbWnd && pnmh->code == TTN_GETDISPINFO)  //
            {
                ...
                switch (TTtext->hdr.idFrom) {
                    case IDM_FILE_OPEN:
                        TTtext->lpszText = STRING_TOOLTIP_OPEN;
                        break;
                    case IDM_FILE_SAVE:
                        TTtext->lpszText = STRING_TOOLTIP_SAVE;
                        break;
                    case IDM_SETTING_AREA:
                        TTtext->lpszText = STRING_TOOLTIP_ROOM;
                        break;
                    case IDM_SETTING_BADGUYS:
                        TTtext->lpszText = STRING_TOOLTIP_BADGUYS;
                        break;
                    case IDM_SETTING_MAP:
                        TTtext->lpszText = STRING_TOOLTIP_MAP;
                        break;
                    case IDM_EMULATOR_NORMALPLAY:
                        TTtext->lpszText = STRING_TOOLTIP_TESTPLAY;
                        break;
                    case IDM_EMULATOR_PAGEPLAY:
                        TTtext->lpszText = STRING_TOOLTIP_PAGETESTPLAY;
                        break;
                    case IDM_EMULATOR_PAGEPLAYHALF:
                        TTtext->lpszText = STRING_TOOLTIP_HALFPOINTTESTPLAY;
                        break;
                    case IDM_EMULATOR_STOP:
                        TTtext->lpszText = STRING_TOOLTIP_STOP;
                        break;
                    case IDM_EMULATOR_TESTPLAYSETTING:
                        TTtext->lpszText = STRING_TOOLTIP_TESTPLAYSETTING;
                        break;
                }
            }
        } break;
        case WM_COMMAND: {
            switch (LOWORD(wParam)) {
                case IDM_FILE_OPEN: {
                    ...
                    ReadFromRegistry(INI_MDIFRAME_FILEPATH, REG_SZ, &curdir,
                    ...
                    fname.nMaxFileTitle = sizeof(filename);
                    fname.lpfnHook = (LPOFNHOOKPROC)OFNHookProc;
                    fname.Flags = OFN_FILEMUSTEXIST | OFN_HIDEREADONLY | OFN_EXPLORER |
                    ...
                    memset(curdir, 0, MAX_PATH * sizeof(TCHAR));
                    memcpy(curdir, FilePath, fname.nFileOffset * sizeof(TCHAR));
                    WriteToRegistry(INI_MDIFRAME_FILEPATH, REG_SZ, curdir,
                                    MAX_PATH * sizeof(TCHAR));

                    memcpy(gFilePath, FilePath, MAX_PATH * sizeof(TCHAR));
                    LoadROMFromFile();
                OPENCANCEL:
                    SendMessage(g_hTbWnd, TB_CHECKBUTTON, (LPARAM)IDM_FILE_OPEN,
                                (WPARAM)MAKELONG(FALSE, 0));
                } break;
                case IDM_FILE_SAVE:
                    if (gblIsROMLoaded) {
                        if (g_fShowMsgOnSave) {
                            if (IDNO == Msg(STRING_CONFIRM_SAVE,
                ...
                case IDM_FILE_CHRLOAD: {
                    ...
                    fname.lpfnHook = (LPOFNHOOKPROC)OFNHookProc;
                    fname.Flags = OFN_FILEMUSTEXIST | OFN_HIDEREADONLY | OFN_EXPLORER |
                                  OFN_ENABLEHOOK | OFN_NOCHANGEDIR | OFN_ENABLETEMPLATE;
                    fname.lpstrDefExt = __T("nes");
                ...
                case IDM_SETTING_AREA: {
                    if (gblIsROMLoaded) {
                        DialogBox(ghInst, __T("ROOMSELECTDLG"), hWnd,
                                  AreaSettingDlgProc);
                        if (g_hTbWnd) {
                            LPARAM dwState;
                            ...
                            // ボタンを元に戻す
                            // Restore button
                            SendMessage(g_hTbWnd, TB_CHECKBUTTON,
                                        (LPARAM)IDM_SETTING_AREA, 0);

                            // 途中ページテストプレイのボタンの有効、無効。
                            // Enable/Disable the button of the middle page test play.
                            dwState = (rm_IsSubRoom()) ? MAKELPARAM(FALSE, 0)
                                                       : MAKELPARAM(TRUE, 0);
                            SendMessage(g_hTbWnd, TB_ENABLEBUTTON,
                                        IDM_EMULATOR_PAGEPLAYHALF, dwState);
                        }
                    }
                } break;
                case IDM_SETTING_MAPHEAD: {
                    if (gblIsROMLoaded)
                        DialogBox(ghInst, __T("MAPCOMHEADEDITDLG"), hWnd,
                                  MapComHeadEditDlgProc);
                } break;
                case IDM_SETTING_MAP: {
                    ...
                    SendMessage(g_hTbWnd, TB_CHECKBUTTON, (LPARAM)IDM_SETTING_MAP,
                                (WPARAM)1);
                } break;
                case IDM_SETTING_BADGUYS: {
                    ...
                    SendMessage(g_hTbWnd, TB_CHECKBUTTON, (LPARAM)IDM_SETTING_BADGUYS,
                                (WPARAM)1);
                } break;
                ...
                case IDM_EDIT_STRINGS:
                    if (gblIsROMLoaded)
                        DialogBox(ghInst, __T("STRINGEDITDLG"), hWnd,
                                  StringEditDlgProc);
                    break;
                case IDM_EDIT_LOOP:
                    if (gblIsROMLoaded)
                        DialogBox(ghInst, __T("LOOPEDITDLG"), hWnd, LoopEditDlgProc);
                    break;
                case IDM_EDIT_AREASORT:
                    if (gblIsROMLoaded)
                        DialogBox(ghInst, __T("AREASORTDLG"), hWnd, AreaSortDlgProc);
                    break;
                case IDM_TOOL_WORLDDATAUPDATE:
                    if (gblIsROMLoaded &&
                        (IDOK == Msg(STRING_CONFIRM_UPDATEWORLD,
                                     MB_OKCANCEL | MB_ICONINFORMATION))) {
                        LPTSTR szBuf = GetTempStringBuffer();
                        UpdateWorldData(TRUE);
                        wsprintf(szBuf,
                                 __T("$9CB4 : %.2xH %.2xH %.2xH %.2xH %.2xH %.2xH "
                                     "%.2xH %.2xH"),
                ...
                case IDM_TOOL_GENERALSETTING:
                    if (gblIsROMLoaded)
                        DialogBox(ghInst, __T("GENERALSETTINGDLG"), hWnd,
                                  GeneralSettingDlgProc);
                    break;
                ...
                case IDM_TOOL_DEMORECORD:
                    if (gblIsROMLoaded) {
                        if (IDCANCEL == Msg(STRING_CONFIRM_DEMORECORD,
                                            MB_OKCANCEL | MB_ICONINFORMATION))
                            break;

                        OpenIcon(GetEmuWndHandle());
                        SendMessage(ghWndMDIClient, WM_MDIACTIVATE,
                ...
                case IDM_EMULATOR_PAGEPLAY: {
                    ...
                    if (GetMapEditMode())
                        ...
                        GetMapData(GETADDRESS_CURRENT_EDITTING, GetSelectedIndex(),
                                   NULL, &iPage);
                ...
                case IDM_EMULATOR_SAVE:
                    if (!gblIsROMLoaded) break;
                    if (SaveEmulatorState()) SetStatusBarText(STRING_STATUSBAR_EMUSAVE);
                    break;
                ...
                case IDM_EMULATOR_TESTPLAYSETTING:
                    if (gblIsROMLoaded) {
                        DialogBox(ghInst, __T("TESTPLAYSETTINGDLG"), hWnd,
                                  TestPlaySettingDlgProc);
                        if (g_hTbWnd)
                            SendMessage(g_hTbWnd, TB_CHECKBUTTON,
                                        (LPARAM)IDM_EMULATOR_TESTPLAYSETTING,
                                        (WPARAM)MAKELONG(FALSE, 0));
                    }
                    break;
        ...
        case WM_INITMENU: {
            CheckMenuRadioItem(GetMenu(hWnd), IDM_SETTING_MAP, IDM_SETTING_BADGUYS,
                               IDM_SETTING_MAP + GetMapEditMode(), MF_BYCOMMAND);
            ...
            EnableMenuItem(GetMenu(hWnd), IDM_EDIT_UNDO, MF_BYCOMMAND | MF_GRAYED);
            {
                ...
                if (undoIsEnabled())
                    EnableMenuItem(GetMenu(hWnd), IDM_EDIT_UNDO,
                                   MF_BYCOMMAND | MF_ENABLED);
            }
        ...
        case WM_CREATE: {
            ...
            // MDI
            if (!ReadFromRegistry(INI_MDIFRAME_OTHERSETTING, REG_DWORD, &dwSetting,
                                  sizeof(DWORD)))
                dwSetting = 0xFFFFFFFF;

            g_hTbWnd = NULL;
            g_hSbWnd = NULL;
            if (dwSetting & MDIFRAME_OTHERSETTING_TOOLBAR)
                g_hTbWnd = CreateMainWindowToolBar(hWnd);
            if (dwSetting & MDIFRAME_OTHERSETTING_STATUSBAR)
                g_hSbWnd = CreateMainWindowStatusBar(hWnd);
            g_fShowMsgOnSave =
                    (dwSetting & MDIFRAME_OTHERSETTING_MSGONSAVE) ? TRUE : FALSE;

            if (lv_ShowLogView()) lv_CreateLogView(hWnd, -1);

            // ObjView
            if (!ReadFromRegistry(INI_OBJECTVIEW_SETTING, REG_DWORD, &dwSetting,
                                  sizeof(DWORD)))
            ...
            ghWndMDIClient = CreateWindowEx(
                    WS_EX_OVERLAPPEDWINDOW, __T("mdiclient"), __T("mdiclient"),
                    WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS |
                            MDIS_ALLCHILDSTYLES,
                    0, 0, 0, 0, hWnd, (HMENU)IDW_MDICLIENT, GetModuleHandle(NULL),
                    (LPTSTR)&ccs);
        ...
        case WM_DESTROY: {
            ...
            WriteToRegistry(INI_MDIFRAME_OTHERSETTING, REG_DWORD, &dwSetting,
                            sizeof(DWORD));

            dwSetting = ObjectViewGetSetting();
            WriteToRegistry(INI_OBJECTVIEW_SETTING, REG_DWORD, &dwSetting,
                            sizeof(DWORD));
...
BOOL RegisterWndClass(HINSTANCE hInstance, int nCmdShow) {
    ...
    /* Create the frame */
    ghWndFrame = CreateWindowEx(WS_EX_ACCEPTFILES, FRAMEWNDCLASSNAME, PROGRAMNAME,
    ...
    if (_tcslen(gFilePath) != 0) LoadROMFromFile();
