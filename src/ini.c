...
BOOL DeleteRegistryEntries() {
    if (ERROR_SUCCESS != RegDeleteKey(HKEY_CURRENT_USER, REGROOTPATH)) return FALSE;
    return TRUE;
}
...
size_t GetAppPathName(LPTSTR lpBuffer, int iBufferSize, LPTSTR lpFileName) {
    ...
    // NOTE :
    // コマンドプロンプトから実行された場合、入力されたコマンド文字列がそのまま取得される。
    //        例えば、WinIPSの置かれているディレクトリ内からコマンドプロンプトで"winips"と起動すると、
    //        "winips"が取得されるので、以下の方法ではだめ。
    // p = GetCommandLine();
    // 実行可能ファイルのフルパス名を取得する
    // NOTE: When executed from the command prompt, the input command character string
    // is acquired as it is. For example, if you start "winips" at the command prompt
    // from within the directory where WinIPS is located, "winips" is acquired, so it
    // can not be done in the following way. p = GetCommandLine(); Get the full path
    // name of the executable file
    GetModuleFileName(GetModuleHandle(NULL), FullPath, MAX_PATH);
    p = FullPath;

    while (*p == __T('"')) p = CharNext(p);
    ...
    // + NULL文字
    // + NULL character
    if (iBufferSize <= cb + (int)sizeof(TCHAR)) return 0;
    ...
    if (lpFileName) {
        if (iBufferSize <= cb + (int)sizeof(TCHAR) + (int)_tcslen(lpFileName)) return 0;
