...
BOOL GetPresetEditorKeys(WORD aEditKeys[], INT_PTR dwID) {
    WORD wKeys[KEYACCEL_NUM_PRESETS][KEYACCEL_NUM_COMMANDS] = {
            ...
            VK_NUMPAD9,     VK_NUMPAD3,
            VK_NUMPAD7,     VK_NUMPAD1,
            CTRLBIT | 0x53, CTRLBIT | 0x5A,
            VK_F2,          VK_F3,
            VK_F4,          VK_F9,
            VK_F10,         VK_F11,
            VK_F12,         VK_F5,
            VK_F7,          VK_F8,
            VK_TAB,         SHIFTBIT | VK_TAB,
            0x57,           0x51,
            0x52,           0x45,
            0x53,           0x58,
            0x5A,           0x43,
    ...
    if (dwID >= KEYACCEL_NUM_PRESETS) return FALSE;
...
BOOL GetEditorVKeys(WORD aEditKeys[]) {
    if (!ReadFromRegistry(INI_EDITOR_KEYS, REG_BINARY, aEditKeys,
...
HACCEL CreateEditCommandAccel() {
    UINT uCmd[KEYACCEL_NUM_COMMANDS] = {IDM_EDITCOMMAND_ADD1TOTYPE,
