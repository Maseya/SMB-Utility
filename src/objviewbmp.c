...
ASSISTBMPDATA g_MapAssistBmpData[] = {
        ...
        0, 0, 0, 0,  0, 6, 0,  0, 8, 0, 0, 8, 0, 0, 7,   0, 0,
        6, 0, 0, 10, 0, 0, 11, 0, 0, 9, 0, 0, 7, 8, -16, 1};
...
ASSISTBMPDATA g_BadGuysAssistBmpData[] = {
        ...
        0,   0,  0,   0,   0,  12,  0,   0,  13,  0,  0,  14,  0,  0,  15,  0, 0,
        12,  0,  -16, 4,   0,  -16, 0,   0,  -16, 1,  0,  -16, 5,  0,  -16, 1, 0,
        -16, 3,  0,   -16, 0,  0,   -16, 1,  0,   -8, 16, 0,   -8, 14,  // パタパタ(跳ねる)
                                                                        // - Scattering
                                                                        // (bouncing)
        0,   -8, 17,  0,   -8, 16,  0,   -8, 14,  0,  0,  18};
    ...
    switch ((lpbBuf[0] & 0x0f)) {
        ...
        default:
            if (!(lpbBuf[1] & 0x70)) {
                ...
                int iItemBlocks[] = {MAPASSISTBMPDATAINDEX_QBLOCKPOWERMUSH,
                ...
                if (bType < 0x09) iRet = iItemBlocks[bType];
...
void BitBltAssistBmp(HDC hDestMemDC, LPASSISTBMPDATA lpABD, int x, int y) {
    if (lpABD && hDestMemDC) {
        BitBlt(hDestMemDC, x + lpABD->iPosDeltaX, y + lpABD->iPosDeltaY,
               ASSISTBMP_1BMPWIDTH, ASSISTBMP_1BMPHEIGHT, g_hAssistBmpDC,
               (lpABD->iBmpIndex % ASSISTBMP_NUMBMPSHORIZONTAL) * ASSISTBMP_1BMPWIDTH,
               (lpABD->iBmpIndex / ASSISTBMP_NUMBMPSHORIZONTAL) * ASSISTBMP_1BMPHEIGHT *
                               2 +
                       ASSISTBMP_1BMPHEIGHT,
               SRCAND);
...
static int GetBadGuysAssistBmpDataIndex(LPBYTE lpbBuf) {
    ...
    switch (lpbBuf[0] & 0x0F) {
        ...
        default: {
            ...
            int iLifts[] = {BADGUYSASSISTBMPDATAINDEX_LIFTTOSS,
