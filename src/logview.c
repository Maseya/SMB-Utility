...
static BOOL AdjustLogView(WORD wBorderPos) {
    ...
    if (!g_hBorderWnd || !g_hLogViewWnd) return FALSE;
    ...
    MoveWindow(g_hLogViewWnd, 0, wBorderPos + LOGVIEW_BORDERWIDTH, w,
               h - (wBorderPos + sh + LOGVIEW_BORDERWIDTH), TRUE);
...
LRESULT CALLBACK LogViewWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        ...
        case WM_SIZE: {
            ...
            MoveWindow(g_hEditWnd, LOGVIEW_EDITBORDER, LOGVIEW_EDITBORDER,
                       rc.right - LOGVIEW_EDITBORDER, rc.bottom - LOGVIEW_EDITBORDER,
                       TRUE);
        } break;
        case WM_CREATE: {
            g_hEditWnd = CreateWindowEx(
                    WS_EX_CLIENTEDGE, __T("EDIT"), NULL,
                    WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL | WS_CLIPSIBLINGS |
                            ES_MULTILINE | ES_READONLY | ES_OEMCONVERT |
                            ES_AUTOHSCROLL | ES_AUTOVSCROLL,
                    0, 0, 1, 1, hWnd, (HMENU)LOGVIEW_EDITID, GetModuleHandle(NULL),
                    NULL);

            SendMessage(g_hEditWnd, WM_SETFONT, (WPARAM)g_hEditFont,
                        MAKELPARAM(TRUE, 0));
...
HWND lv_CreateLogView(HWND hWndParent, int nHeight) {
    ...
    hWnd = CreateWindowEx(0, BORDERWNDCLASSNAME, STRING_EMPTY,
                          WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS, 0, nHeight,
                          rc.right - rc.left, LOGVIEW_BORDERWIDTH, hWndParent, NULL,
                          hInstance, NULL);

    g_hBorderWnd = hWnd;

    hWnd = CreateWindowEx(0, LOGVIEWWNDCLASSNAME, NULL,
                          WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, 0,
                          nHeight + LOGVIEW_BORDERWIDTH, rc.right - rc.left, 1,
...
BOOL lv_OutputString(LPTSTR szText, DWORD dwFlag) {
    ...
    if (g_hEditWnd) SetLogViewTexts();

    if (dwFlag & LOGVIEW_OUTPUTSTRING_BEEP) MessageBeep(MB_OK);
...
VOID lv_AdjustLogView() {
    ...
    if (g_hLogViewWnd) PostMessage(GetParent(g_hLogViewWnd), WM_SIZE, 0, 0);
}

BOOL lv_ShowLogView() { return g_fShowLogView; }

HWND lv_GetBorderWnd() { return g_hBorderWnd; }

HWND lv_GetLogViewWnd() { return g_hLogViewWnd; }
