...
LRESULT CALLBACK RoomSelectDlgProc(HWND hDlg, UINT message, WPARAM wParam,
                                   LPARAM lParam) {
    switch (message) {
        case WM_INITDIALOG: {
...
INT_PTR RoomSelectDialogBox(HWND hWnd, LPROOMSELECT lpRoomSelect) {
    ...
    blRet = DialogBoxParam(GetModuleHandle(NULL), __T("SENDOBJECTDLG"), hWnd,
                           RoomSelectDlgProc, (LPARAM)lpRoomSelect);
