...
#define MAPVIEW_WINDOWHEIGHT                                                    \
    (MAPVIEW_CLIENTHEIGHT + XPOSLABEL_HEIGHT + GetSystemMetrics(SM_CYCAPTION) + \
     GetSystemMetrics(SM_CYSIZEFRAME) * 2)
#define MAPVIEW_VISIBLEHEIGHT (OBJVIEW_CHARACTER_WIDTH * 14)
#define ONE_PAGE_SIZE \
    (NES_SCREENSIZEX + GetSystemMetrics(SM_CXSIZEFRAME) * 2 - 1 + YPOSLABEL_WIDTH)
...
DWORD ObjectViewGetSetting() {
    ...
    if (g_fNotDrawAssistBmp) dwSetting |= OBJVIEWSETTING_NOTDRAWASSISTBMP;

    if (g_blIsXPosLineDraw) dwSetting |= OBJVIEWSETTING_DRAWXPOSLINE;

    if (g_blIsYPosLineDraw) dwSetting |= OBJVIEWSETTING_DRAWYPOSLINE;
...
static void RefreshMapViewWindowTitle() {
    ...
    if (gblIsROMLoaded) {
        ...
        if (giWndPages)
            wsprintf(cWndTitle, STRING_OBJVIEW_TITLE2, STRING_WINDOW_OBJVIEW,
                     giMapViewPageBase, giMapViewPageBase + giWndPages);
        else
            wsprintf(cWndTitle, STRING_OBJVIEW_TITLE, STRING_WINDOW_OBJVIEW,
                     giMapViewPageBase);
        if (lstrcmp(cCurWndTitle, cWndTitle)) SetWindowText(ghMapViewWnd, cWndTitle);
    } else
...
void EnsureMapViewCursoleVisible() { g_iCursoleEnable = 0; }

static void SetMapViewPage(int iPage) { giMapViewPage = iPage; }
...
BOOL ShadeRect(HDC hDC, int x, int y, COLORREF crHighlightColor) {
    ...
    WORD Bits[8] = {0x0055, 0x00aa, 0x0055, 0x00aa, 0x0055, 0x00aa, 0x0055, 0x00aa};
    ...
    if ((hBrushBitmap == NULL) || (hBitmap == NULL) || (hMemDC == NULL) ||
        (hBrush == NULL)) {
    ...
    BitBlt(hMemDC, 0, 0, nWidth, nHeight, hDC, x, y, SRCAND);
...
BOOL PrepareGauge(HWND hWnd, HDC* lphDC, HBITMAP* lphBmp, UINT X, UINT Y,
                  HBITMAP* lphPrevBmp) {
    HDC hdc;
...
HWND CreateObjectViewTooltip() {
    ...
    hwndTT = CreateWindowEx(0, TOOLTIPS_CLASS, (LPTSTR)NULL, 0, CW_USEDEFAULT,
                            CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, ghMapViewWnd,
...
BOOL AddToolToObjectViewToolTip(int x, int y, LPTSTR lpText, UINT id) {
    ...
    if (!SendMessage(ghToolTip, TTM_ADDTOOL, 0, (LPARAM)(LPTOOLINFO)&ti)) return FALSE;
...
void DrawAssistObjectMap() {
    ...
    if (!gblIsROMLoaded || giMapViewPageBase < 0 || giWndPages < 0) return;

    if (!DeleteAllToolObjectViewToolTip()) return;
    ...
    if (!GetMapEditMode()) {
        if (MapSeekFirst(&ObjSeek, GETADDRESS_CURRENT_EDITTING)) {
            for (;;) {
                if ((DWORD)giMapViewPageBase <= ObjSeek.dwPage &&
                    ObjSeek.dwPage <=
                            (DWORD)giMapViewPageBase + (DWORD)giWndPages + 1) {
                    // 最後のページの横位置が負のオブジェクトのために必要
                    // The lateral position of the last page is required for negative
                    // objects
                    uBaseX = (ObjSeek.dwPage - giMapViewPageBase >= 0)
                                     ? (ObjSeek.dwPage - giMapViewPageBase) * 256
                                     : 0;
                    ...
                    if (tx != x || ty != y)
                        wsprintf(lpText2, STRING_OBJVIEW_TOOLTIP, ObjSeek.dwPage,
                                 (x - uBaseX) / OBJVIEW_CHARACTER_WIDTH,
                                 y / OBJVIEW_CHARACTER_WIDTH - 1, lpText);
                    else
                        lpText2 = lpText;
                    AddToolToObjectViewToolTip(XPOSLABEL_HEIGHT + x,
                                               YPOSLABEL_WIDTH + y, lpText2, id++);
            ...
            for (;;) {
                if ((DWORD)giMapViewPageBase <= ObjSeek.dwPage &&
                    ...
                    if (BadGuysIsHardObject(ObjSeek.pbData)) {
                        SetRect(&rcInv, x, y, x + OBJVIEW_BADGUYSHARD_RECTWIDTH,
                                y + OBJVIEW_CHARACTER_WIDTH);
...
BOOL DrawHalfPageMark() {
    ...
    if (giMapViewPageBase > halfpage || giMapViewPageBase + giWndPages < halfpage)
        return FALSE;

    uBaseX = (halfpage - giMapViewPageBase) * 256;

    ABD.iBmpIndex = 3;
    ABD.iPosDeltaX = ABD.iPosDeltaY = 0;
...
static void RedrawMapViewWnd() {
    ...
    BitBlt(hdc, YPOSLABEL_WIDTH, XPOSLABEL_HEIGHT, GetSystemMetrics(SM_CXSCREEN),
           MAPVIEW_CLIENTHEIGHT, ghMemdcMapViewWnd, 0, 0, SRCCOPY);
    ReleaseDC(ghMapViewWnd, hdc);

    if (IsMapViewCursoleVisible()) DrawMapViewCursor(GetMapEditMode());
}
...
void ClearObjectViewBackBuffer() {
    ...
    if (ghMemdcMapViewWnd)
        FillRect(ghMemdcMapViewWnd, &rc, GetSysColorBrush(COLOR_WINDOW));
}
...
void UpdateObjectView(DWORD dwUpdateFlag) {
    ...
    } else if (giMapViewPageBase <= giMapViewPage &&
               giMapViewPage <= giMapViewPageBase + giWndPages) {
    ...
    for (n = 0; n <= giWndPages; n++) {
        if (RunEmulatorViewPage(GETADDRESS_CURRENT_EDITTING, giMapViewPageBase + n)) {
            TransferFromEmuBackBuffer(ghMemdcMapViewWnd, n * NES_SCREENSIZEX, 0,
                                      NES_SCREENSIZEX - 1, NES_SCREENSIZEY - 16, FALSE);
        }
    }
...
BOOL UpdateObjectViewCursole() {
    ...
    if (GetMapEditMode()) {
        if (-1 != GetBadGuysData(GETADDRESS_CURRENT_EDITTING, GetSelectedIndex(), bBuf,
                                 &iPage)) {
...
void ChangeMapViewScreenSize(int iPages) {
    ...
    SetWindowPos(ghMapViewWnd, NULL, 0, 0,
                 ONE_PAGE_SIZE + NES_SCREENSIZEX * (iPages - 1) - 1,
                 MAPVIEW_WINDOWHEIGHT /*rcWnd.bottom-rcWnd.top*/,
...
static BOOL ObjviewMousePos2ObjPos(WORD xPos, WORD yPos,
                                   LPOBJVIEWMOUSEINPUT lpMouseInput) {
    if (gblIsROMLoaded && yPos > XPOSLABEL_HEIGHT && xPos > YPOSLABEL_WIDTH) {
        lpMouseInput->X = xPos;
        lpMouseInput->Y = yPos;
...
static BOOL ObjviewInvertObject(HWND hWnd, LPOBJVIEWMOUSEINPUT lpMouseInput) {
    ...
    if (!ObjviewObjPos2MousePos(lpMouseInput, &pt)) return FALSE;
...
LRESULT FAR PASCAL MapViewWndProc(HWND hWnd, UINT message, WPARAM wParam,
                                  LPARAM lParam) {
    ...
    switch (message) {
            ...
            switch (LOWORD(wParam)) {
                    ...
                    switch (HIWORD(wParam)) {
                        case BN_CLICKED:
                            SendMessage(hWnd, WM_COMMAND,
                                        MAKEWPARAM(IDM_MAPVIEW_NEXTPAGE, 0), 0);
                            SetFocus(hWnd);
                            break;
                    }
                } break;
                case IDW_REWVPAGEWINDOW: {
                    switch (HIWORD(wParam)) {
                        case BN_CLICKED:
                            SendMessage(hWnd, WM_COMMAND,
                                        MAKEWPARAM(IDM_MAPVIEW_PREVPAGE, 0), 0);
                            SetFocus(hWnd);
                            break;
                    }
                } break;
                ...
                case IDM_MAPVIEW_TESTPLAY: {
                    ...
                    SendMessage(GETCLIENTWNDHANDLE(hWnd), WM_MDIACTIVATE,
                                (WPARAM)GetEmuWndHandle(), 0);
                } break;
        ...
        case WM_INITMENU: {
            ...
            if (!gblIsROMLoaded) {
                EnableMenuItem(hMenuInit, IDM_MAPVIEW_NEXTPAGE,
                               MF_BYCOMMAND | MF_GRAYED);
                EnableMenuItem(hMenuInit, IDM_MAPVIEW_PREVPAGE,
                               MF_BYCOMMAND | MF_GRAYED);
                EnableMenuItem(hMenuInit, IDM_MAPVIEW_SENDOBJECT,
                               MF_BYCOMMAND | MF_GRAYED);
                EnableMenuItem(hMenuInit, IDM_MAPVIEW_SHOWEDITDIALOG,
                               MF_BYCOMMAND | MF_GRAYED);
                EnableMenuItem(hMenuInit, IDM_MAPVIEW_TESTPLAY,
                               MF_BYCOMMAND | MF_GRAYED);
                EnableMenuItem(hMenuInit, IDM_MAPVIEW_SWITCHEDITMODE,
                               MF_BYCOMMAND | MF_GRAYED);
            }
        ...
        case WM_DRAWITEM: {
            ...
            LPDRAWITEMSTRUCT lpdis =
                    (LPDRAWITEMSTRUCT)lParam;  // item-drawing information
            RECT rc;

            GetClientRect(lpdis->hwndItem, &rc);
            switch (idCtl) {
                case IDW_YPOSWINDOW: {
                    BitBlt(lpdis->hDC, 0, 0, YPOSLABEL_WIDTH, MAPVIEW_CLIENTHEIGHT,
                           ghMemdcYPos, 0, 0, SRCCOPY);
                } break;
                case IDW_XPOSWINDOW: {
                    int n, iPages = rc.right / NES_SCREENSIZEX + 1;
                    for (n = 0; n < iPages; n++)
                        BitBlt(lpdis->hDC, n * NES_SCREENSIZEX, 0, XPOSLABEL_WIDTH,
                               XPOSLABEL_HEIGHT, ghMemdcXPos, 0, 0, SRCCOPY);
                } break;
            }
        } break;
        case WM_SIZE: {
            ...
            MoveWindow(ghYPosWnd, 0, XPOSLABEL_HEIGHT, YPOSLABEL_WIDTH,
                       wHeight - XPOSLABEL_HEIGHT, TRUE);
            MoveWindow(ghXPosWnd, YPOSLABEL_WIDTH, 0, wWidth - YPOSLABEL_WIDTH,
                       XPOSLABEL_HEIGHT, TRUE);
        ...
        case WM_LBUTTONDOWN: {
            ...
            // ポップアップメニューが　
            // 0：非表示、-1：表示→選択なし、それ以外：表示→選択あり The pop-up menu is
            // 0: non-display, -1: display → no selection, otherwise: display → selected
            ...
            if (GetMapEditMode())
                iNewIndex = GetBadGuysDataIndex(GETADDRESS_CURRENT_EDITTING, &sGetIndex,
                                                MouseInput.Page, FALSE);
            else
                iNewIndex = GetMapDataIndex(GETADDRESS_CURRENT_EDITTING, &sGetIndex,
                                            MouseInput.Page, FALSE);

            if (iNewIndex == GETDATAINDEX_ERROR_NOTFOUND) break;
                ...
                if (MK_CONTROL & fwKeys) {
                    ...
                    if (!GetMapEditMode()) {
                        while (N < sGetIndex.nNumSamePos) {
                            GetMapData(GETADDRESS_CURRENT_EDITTING,
                                       sGetIndex.iIndex + N, bData, NULL);
                            FormatMapString(bData, szMenuText);
                            uMenuFlag = (sGetIndex.iIndex + N == GetSelectedIndex())
                                                ? MF_BYPOSITION | MF_CHECKED
                                                : MF_BYPOSITION;
                            InsertMenu(hSelMenu, N, uMenuFlag,
                                       ID_OBJVIEW_OBJSELECT_BASE + sGetIndex.iIndex + N,
                                       szMenuText);
                            N++;
                        }
                    } else {
                        while (N < sGetIndex.nNumSamePos) {
                            GetBadGuysData(GETADDRESS_CURRENT_EDITTING,
                                           sGetIndex.iIndex + N, bData, NULL);
                    ...
                    uPopMenuSelected = TrackPopupMenu(hSelMenu, TPM_RETURNCMD, pt.x,
                                                      pt.y, 0, hWnd, NULL);
                    DestroyMenu(hSelMenu);

                    if (uPopMenuSelected) {
                        SetSelectedItem(uPopMenuSelected - ID_OBJVIEW_OBJSELECT_BASE,
                                        TRUE);
                    } else
                        uPopMenuSelected = -1;
                } else {
        ...
        case WM_MOUSEMOVE: {
            ...
            if ((fwKeys & MK_LBUTTON) && fObjectIsGrabed &&
                ...
                if (GrabedObject.Page == MouseInput.Page) {
                    if (GrabedObject.Y == MouseInput.Y) {
                        if (GrabedObject.X - 1 == MouseInput.X) {
                            if (MapEditCommand(IDM_EDITCOMMAND_LEFT)) blUpdate = TRUE;
                        } else if (GrabedObject.X + 1 == MouseInput.X) {
                            if (MapEditCommand(IDM_EDITCOMMAND_RIGHT)) blUpdate = TRUE;
                        }
                    } else if (GrabedObject.X == MouseInput.X) {
                        if (GrabedObject.Y + 1 == MouseInput.Y) {
                            if (MapEditCommand(IDM_EDITCOMMAND_DOWN)) blUpdate = TRUE;
                        } else if (GrabedObject.Y - 1 == MouseInput.Y) {
                            if (MapEditCommand(IDM_EDITCOMMAND_UP)) blUpdate = TRUE;
                        }
                    }
                } else {
                    if (GrabedObject.Page - 1 == MouseInput.Page &&
                        MouseInput.X == 15 && !GrabedObject.X) {
                        if (MapEditCommand(IDM_EDITCOMMAND_LEFT)) blUpdate = TRUE;
                    } else if (GrabedObject.Page + 1 == MouseInput.Page &&
                               MouseInput.X == 0 && GrabedObject.X == 15) {
                        if (MapEditCommand(IDM_EDITCOMMAND_RIGHT)) blUpdate = TRUE;
                    }
                }
                if (blUpdate) {
        ...
        case WM_CREATE: {
            ...
            ghYPosWnd =
                    CreateWindowEx(0, __T("STATIC"), NULL,
                                   WS_CHILD | WS_VISIBLE | SS_OWNERDRAW | SS_NOTIFY, 0,
                                   XPOSLABEL_HEIGHT, YPOSLABEL_WIDTH, 0, hWnd,
                                   (HMENU)IDW_YPOSWINDOW, GetModuleHandle(NULL), NULL);
            PrepareGauge(ghYPosWnd, &ghMemdcYPos, &ghBitmapYPos, YPOSLABEL_WIDTH,
                         MAPVIEW_CLIENTHEIGHT, &ghPrevBitmapYPos);
            DrawYPosGauge();

            ghXPosWnd =
                    CreateWindowEx(0, __T("STATIC"), NULL,
                                   WS_CHILD | WS_VISIBLE | SS_OWNERDRAW | SS_NOTIFY,
                                   YPOSLABEL_WIDTH, 0, 0, XPOSLABEL_HEIGHT, hWnd,
                                   (HMENU)IDW_XPOSWINDOW, GetModuleHandle(NULL), NULL);
            PrepareGauge(ghXPosWnd, &ghMemdcXPos, &ghBitmapXPos, XPOSLABEL_WIDTH,
                         XPOSLABEL_HEIGHT, &ghPrevBitmapXPos);
            DrawXPosGauge();

            ghRPBtnWnd = CreateWindow(__T("BUTTON"), STRING_EMPTY,
                                      WS_CHILD | WS_VISIBLE | BS_BITMAP | BS_CENTER, 0,
                                      0, YPOSLABEL_WIDTH / 2, XPOSLABEL_HEIGHT, hWnd,
                                      (HMENU)IDW_REWVPAGEWINDOW, GetModuleHandle(NULL),
                                      NULL);
            hBmp = (HBITMAP)LoadImage(GetModuleHandle(NULL), __T("LF_IMG"),
                                      IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
            SendMessage(ghRPBtnWnd, BM_SETIMAGE, (WPARAM)IMAGE_BITMAP, (LPARAM)hBmp);

            ghFPBtnWnd = CreateWindow(__T("BUTTON"), STRING_EMPTY,
                                      WS_CHILD | WS_VISIBLE | BS_BITMAP | BS_CENTER,
                                      YPOSLABEL_WIDTH / 2, 0, YPOSLABEL_WIDTH / 2,
                                      XPOSLABEL_HEIGHT, hWnd, (HMENU)IDW_FWDPAGEWINDOW,
                                      GetModuleHandle(NULL), NULL);
            hBmp = (HBITMAP)LoadImage(GetModuleHandle(NULL), __T("RG_IMG"),
                                      IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
        ...
        case WM_DESTROY: {
            ...
            hBmp = (HBITMAP)SendMessage(ghRPBtnWnd, BM_GETIMAGE, (WPARAM)IMAGE_BITMAP,
                                        0);
            DeleteObject(hBmp);
            DestroyWindow(ghRPBtnWnd);

            hBmp = (HBITMAP)SendMessage(ghFPBtnWnd, BM_GETIMAGE, (WPARAM)IMAGE_BITMAP,
                                        0);
            DeleteObject(hBmp);
...
HWND CreateMapViewWnd(HINSTANCE hInstance, HWND hWndMDIClient) {
    ...
    // Window menu.
    hWnd = CreateMDIWindow(MAPVIEWWNDCLASSNAME, STRING_WINDOW_OBJVIEW,
                           WS_SYSMENU | WS_MINIMIZEBOX | WS_CAPTION | WS_THICKFRAME |
                                   WS_CLIPCHILDREN | WS_VISIBLE,
                           60,  // CW_USEDEFAULT,
                           40,  // CW_USEDEFAULT,
                           GetSystemMetrics(SM_CXSCREEN) / 2,
                           GetSystemMetrics(SM_CYSCREEN) / 2, hWndMDIClient, hInstance,
                           0);
